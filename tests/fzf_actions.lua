...
do
  ...
  local _, output = check("tabs", "current", defaults, { "\r" }, true)
  local result = results(output)[1]
  assert(result.query == "" and result.key == "" and result.target == "fixture-id")
  _, output = check("tabs", "current", defaults, { "nomatch", "\19", "\27" }, false)
  result = results(output)[1]
  assert(result.query == "nomatch" and result.key == "ctrl-s" and result.selected_id == nil)
end
for _, key in ipairs({ "refresh", "toggle_preview", "tabs_current", "tabs_all", "spaces", "agents_current", "agents_all", "panes_tab", "panes_current", "panes_all" }) do
  settings.keys[key] = {}
end
for _, variant in ipairs({ { "tabs", "current" }, { "tabs", "all" }, { "workspaces", "all" },
  { "agents", "current" }, { "agents", "all" }, { "panes", "tab" }, { "panes", "current" }, { "panes", "all" } }) do
...
print("Real fzf: all eight variants accept through both remapped aliases and close with all optional actions disabled; built-in lifecycle guards pass")
...
assert(not narrow:find("all panes", 1, true), "narrow footer should clip trailing pane hints")
local resized = check("panes", "tab", { preview = { enabled_by_default = false } },
  { "", "", "", "", "\27" }, false, nil, nil, "layout-resize")
assert(resized:find("all panes", 1, true), "resizing should reveal the retained second-row hints")
...
do
  ...
  _, output = check("tabs", "current", memory_settings,
    { "label", "\27[B", "\19", "\18", "", "\21fixture", "\19", "\18", "", "\r" }, true, nil, "fixture-id", "memory-reorder")
  exits = results(output)
...
do
  local remapped = { keys = { accept = { "alt-v" }, close = { "alt-x" }, tabs_all = { "alt-t" }, spaces = { "alt-s" } } }
...
local pane_scopes = { { "tab", "\27" .. "1" }, { "current", "\27" .. "2" }, { "all", "\27" .. "3" } }
...
do
  local _, output = check("panes", "all", {},
    { "label", "\27[B", "\12", "\12", "", "\27[A", "\19", "\27" .. "3", "", "\r" },
    true, nil, "fixture-id", "memory-retry")
  assert(results(output)[1].selected_id == "fixture-id")
  ...
  _, output = check("panes", "tab", {},
    { "label", "\27[B", "\19", "\27" .. "1", "\r", "\21renamed", "\19", "\27" .. "1", "", "\r" },
    true, nil, "fixture-next", "memory-pending")
  local result = results(output)[3]
...
do
  local pane_settings = { columns = { panes_tab = { "title" }, panes_current = { "directory" }, panes_all = { "status" } },
  ...
  local screen = check("panes", "tab", pane_settings,
    { "beta", "\27" .. "2", "", "", "second", "\r" }, true, nil, "fixture-next", "pane-columns")
  assert(screen:find("EXACT_fixture-next", 1, true))
  check("panes", "current", pane_settings,
    { "nomatch", "\27" .. "3", "", "", "blocked", "\r" }, true, nil, "fixture-next", "pane-columns")
  ...
  local _, output = check("panes", "tab", {},
    { "beta", "\12", "", "", "\27" .. "3", "", "", "beta", "\r" }, true, nil, "fixture-next", "pane-move")
  assert(not results(output)[1].selected_id and results(output)[1].query == "beta")
  local remapped = { keys = { accept = { "alt-v" }, panes_tab = { "alt-q" }, panes_all = { "alt-w" } } }
  check("panes", "tab", remapped,
    { "label", "\27[B", "\27w", "", "fixture", "\27q", "", "", "\27v" },
    true, nil, "fixture-next", "memory-reorder")
end
