...
local snapshot = {
  ...
  panes = { { pane_id = "p1", label = "review", cwd = "/one" }, { pane_id = "p2", label = "done", cwd = "/two" } },
...
core.candidates = function(kind, scope, settings) return core.snapshot_candidates(snapshot, kind, scope, "w1", settings) end
for _, variant in ipairs(variants) do
  ...
  for _, text in ipairs({ "{}", '{"theme":{"name":"catppuccin-latte"}}', '{"theme":{"name":"rose-pine"}}',
    ...
    if variant[1] == "agents" then assert(contents:find(themes.ansi(settings.roles.annotation) .. "[", 1, true)) end
...
print("Themed rendering: five variants, dark/light/terminal/custom roles, refresh stability, unchanged text/alignment/IDs/filter targets and preview chrome OK")
